...
<Exercise client:load
  ...
  description="Modify the Hello World program so that it prints exactly: `Hello, C++!`"
  type="code-output"
  expectedOutput="Hello, C++!"
  starterCode={`#include <iostream>

int main() {
    // Change the line below so output is: Hello, C++!
    std::cout << "Hello, World!" << std::endl;
    return 0;
}`}
  hints={["Replace the text inside the double quotes", "The output must be exactly: Hello, C++!", "Do not change any other part of the program"]}
  explanation="std::cout prints whatever text is inside the double quotes exactly as written. Simply replace the string content to change what appears on screen."
  lang="en"
/>

<Exercise client:load
  id="u1l1-ex2"
  title="Purpose of return 0"
  description="What is the purpose of the `return 0;` statement inside the `main()` function?"
  type="multiple-choice"
  options={[
    "It ends a loop in the program",
    "It tells the operating system that the program ran successfully",
    "It declares a variable of type int",
    "It starts execution of the program from the beginning"
  ]}
  correctIndex={1}
  hints={["The number 0 is a universal convention for 'everything went fine'", "main() returns a value to the operating system"]}
  explanation="return 0; returns the value 0 to the operating system, signaling that the program finished successfully. A non-zero return value indicates that something went wrong."
  lang="en"
/>
