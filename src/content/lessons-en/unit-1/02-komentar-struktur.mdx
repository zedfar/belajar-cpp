...
<Exercise client:load
  id="u1l2-ex1"
  title="Commented-Out Code Output"
  description="A program has one line commented out using `//`. What output does it produce?"
  ...
  correctIndex={1}
  hints={["Lines starting with // are ignored by the compiler", "Look at which lines are active and which are commented out"]}
  explanation='The line `// std::cout << "Step 2"` is a comment — the compiler ignores it entirely. Only Step 1 and Step 3 are active statements, so only those two lines are printed.'
  lang="en"
/>

<Exercise client:load
  id="u1l2-ex2"
  title="Identify the Comment Style"
  description="Which syntax is used to write a multi-line comment in C++?"
  type="multiple-choice"
  options={[
    "// comment starts here ... // comment ends here",
    "# This is a comment",
    "/* comment text that spans multiple lines */",
    "-- this is a comment"
  ]}
  correctIndex={2}
  hints={["Single-line comments use //", "Multi-line comments need a pair of opening and closing markers"]}
  explanation="Multi-line comments in C++ are enclosed between /* and */. Everything between these markers is ignored by the compiler, no matter how many lines it spans. Single-line comments use // and only cover the rest of that one line."
  lang="en"
/>
