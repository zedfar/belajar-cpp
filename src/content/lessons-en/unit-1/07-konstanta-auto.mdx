...
<Exercise client:load
  id="u1l7-ex1"
  title="Correct Constant Declaration"
  description="Which is the correct way to declare a constant named PI with value 3.14?"
  ...
  correctIndex={1}
  hints={["The const keyword must appear before the data type", "The data type must still be written even when using const"]}
  explanation="The correct syntax is: const data_type name = value; So `const double PI = 3.14;` is the proper constant declaration. Without the data type, the compiler cannot allocate the correct amount of memory."
  lang="en"
/>

<Exercise client:load
  id="u1l7-ex2"
  title="Circle Circumference with a Constant"
  description="Complete the program so it prints the circumference of a circle. Output must be exactly: `Circumference: 62.8`"
  type="code-output"
  expectedOutput="Circumference: 62.8"
...
int main() {
    ...
    // Print: Circumference: 62.8

    return 0;
}`}
  hints={["circumference = 2 * PI * radius", "2 * 3.14 * 10.0 = 62.8", `Use std::cout << "Circumference: " << circumference << std::endl;`]}
  explanation="The constant PI is used in the calculation: circumference = 2 * PI * r = 2 * 3.14 * 10 = 62.8. Using const ensures PI cannot be accidentally changed."
  lang="en"
/>
