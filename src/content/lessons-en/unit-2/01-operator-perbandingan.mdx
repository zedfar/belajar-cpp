...
![Comparison operators examples: 5 > 3 is true, 7 == 7 is true, 2 > 8 is false](/images/operator-perbandingan.svg)
...
<Exercise client:load
  id="u2l1-ex1"
  title="Correct Comparison Operator"
  description="You want to check whether the value of variable `x` is equal to 10. Which operator should you use?"
  type="multiple-choice"
  options={["x = 10", "x == 10", "x != 10", "x := 10"]}
  correctIndex={1}
  hints={["= is assignment (storing a value), not comparison", "== is used to compare two values"]}
  explanation="== is the comparison operator that checks whether two values are equal. The = operator (single equals sign) is used for assignment (storing a value), not comparing. This is one of the most common mistakes in C++!"
  lang="en"
/>

<Exercise client:load
  id="u2l1-ex2"
  title="Comparison Result Output"
  description="Complete the program so it prints the result of comparing whether `score` is greater than or equal to `passingGrade`. Output must be: `1` (meaning true — 85 is indeed &gt;= 75)."
  type="code-output"
  expectedOutput="1"
...
int main() {
    ...
    // Print the comparison result: is score >= passingGrade?
    // Output must be: 1

    return 0;
}`}
  hints={["Use the >= operator for comparison", "std::cout << (score >= passingGrade) << std::endl;", "Wrap the comparison in parentheses when used inside cout"]}
  explanation="The expression (score >= passingGrade) produces a bool. Because 85 >= 75 is true, the result is true which cout displays as 1."
  lang="en"
/>
