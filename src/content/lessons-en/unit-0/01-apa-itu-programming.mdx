...
![C++ compilation flow: Source Code → Compiler → Program → Output on screen](/images/compilation-flow.svg)
...
<Exercise client:load
  ...
  description="Which of the following best describes the role of a compiler in C++ development?"
  type="multiple-choice"
  options={[
    "It runs the program and displays output on the screen",
    "It translates source code into machine language that the computer can execute",
    "It writes the code automatically based on your description",
    "It connects the program to the internet"
  ]}
  correctIndex={1}
  hints={["Think about the analogy: you write in English but the recipient only understands Japanese", "A compiler is like a translator between two languages"]}
  explanation="A compiler translates the C++ source code you write into binary machine language that the processor can execute. It also checks for errors in your code, acting like a 'spell checker' for programs."
  lang="en"
/>
...
<Exercise client:load
  id="u0l1-ex2"
  title="C++ Name Origin"
  description="The name 'C++' is a programmer joke. In C, the `++` operator means 'increment by 1'. What does the name C++ imply about the language?"
  type="multiple-choice"
  options={[
    "C++ is twice as fast as C",
    "C++ is an improved version of C — 'C plus one'",
    "C++ adds internet features to C",
    "C++ was created after C# and Java"
  ]}
  correctIndex={1}
  hints={["++ in C means 'add 1 to the value'", "Think of it as a version increment"]}
  explanation="C++ was created by Bjarne Stroustrup as an extension of the C language. The name 'C++' uses the C increment operator (++) to imply it is 'C plus one' — an improved, enhanced version of C."
  lang="en"
/>
