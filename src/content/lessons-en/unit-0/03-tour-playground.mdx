...
<Exercise client:load
  ...
  description="The compiler shows this error: `error: expected ';' before 'return'`. What is most likely the cause?"
  type="multiple-choice"
  options={[
    "The #include line is missing",
    "A semicolon is missing at the end of a statement before return",
    "The word 'return' is misspelled",
    "The main function is not declared"
  ]}
  correctIndex={1}
  hints={["In C++, every statement must end with a semicolon", "The error says it expected a semicolon before 'return'", "The problem is on the line just before return"]}
  explanation="The error 'expected ';' before 'return'' means the compiler reached the 'return' keyword but was expecting a semicolon first. This typically means the previous statement is missing its terminating semicolon (;)."
  lang="en"
/>
...
<Exercise client:load
  ...
  description="How many lines of output does this program produce?"
  type="code-output"
  expectedOutput={`Hello! I'm learning C++!
This is the second line.
And this is the third!`}
...
int main() {
  ...
  hints={["Each std::cout with std::endl produces one line of output", "Count the number of cout statements"]}
  explanation="Each std::cout statement sends text to the screen, and std::endl moves to a new line. Three cout statements produce three lines of output, printed in order from top to bottom."
  lang="en"
/>
