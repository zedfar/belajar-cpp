...
![Variabel sebagai kotak penyimpanan berlabel: umur (int=17), nama (string="Budi"), tinggi (double=170.5)](/images/variabel-boxes.svg)
...
<Exercise
  ...
  description="Tipe data apa yang paling tepat untuk menyimpan nilai <code>3.14</code> (bilangan desimal)?"
  type="multiple-choice"
  options={["int", "char", "double", "bool"]}
  correctIndex={2}
  hints={["int hanya untuk bilangan bulat", "double dirancang untuk menyimpan bilangan dengan titik desimal"]}
  explanation="double adalah tipe data untuk bilangan desimal (floating point) dengan presisi tinggi. int hanya menyimpan bilangan bulat — nilai 3.14 akan dipotong menjadi 3 jika disimpan dalam int."
  lang="id"
/>

<Exercise
  id="u1l3-ex2"
  title="Deklarasi dan Output Variabel"
  description="Lengkapi program berikut agar mendeklarasikan variabel dan mencetak output yang benar."
  type="code-output"
  expectedOutput="Umur: 16"
  starterCode={`#include <iostream>

int main() {
    // Deklarasikan variabel umur bertipe int dengan nilai 16
    // lalu cetak: Umur: 16

    return 0;
}`}
  hints={["Tulis: int umur = 16;", "Gunakan std::cout << untuk mencetak", 'Format output: std::cout << "Umur: " << umur << std::endl;']}
  explanation="Variabel int dideklarasikan dengan format: int nama = nilai; Lalu ditampilkan menggunakan std::cout << nama."
  lang="id"
/>
