...
<Exercise client:load
  id="u1l4-ex1"
  title="Arah Operator I/O"
  description="Operator manakah yang digunakan untuk <strong>menerima input</strong> dari keyboard ke dalam variabel?"
  type="multiple-choice"
  options={["cout <<", "cin >>", "cout >>", "cin <<"]}
  correctIndex={1}
  hints={["cin adalah character input stream", "Arah panah >> menjauh dari cin, menuju variabel"]}
  explanation="cin >> digunakan untuk menerima input dari keyboard. Panah >> menunjuk ke arah variabel tujuan — data 'diambil dari' cin dan disimpan ke variabel."
  lang="id"
/>

<Exercise client:load
  id="u1l4-ex2"
  title="Program Output Sederhana"
  description="Lengkapi program agar mencetak dua baris output: <code>Nama: Andi</code> lalu <code>Kota: Jakarta</code>"
  type="code-output"
...
int main() {
    ...
    // Tampilkan kedua variabel dengan format yang benar

    return 0;
}`}
  hints={['Gunakan std::cout << "Nama: " << nama << std::endl;', "Lakukan hal yang sama untuk kota", "Pastikan format persis: Nama: Andi dan Kota: Jakarta"]}
  explanation="std::cout dapat menggabungkan string literal dan variabel menggunakan operator <<. Setiap cout dengan endl menghasilkan satu baris output."
  lang="id"
/>
