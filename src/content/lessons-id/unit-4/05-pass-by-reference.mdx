...
<Exercise client:load
  ...
  description="Perhatikan kode ini: void tambahSepuluh(int angka) { angka = angka + 10; } — int nilai = 5; tambahSepuluh(nilai); — Apa nilai variabel 'nilai' setelah pemanggilan fungsi?"
  type="multiple-choice"
  options={["15, karena fungsi menambahkan 10", "5, karena pass by value hanya mengubah salinan", "0, karena variabel di-reset", "Compiler error, fungsi tidak mengembalikan nilai"]}
  correctIndex={1}
  hints={["Pass by value membuat salinan dari variabel", "Perubahan pada salinan tidak mempengaruhi variabel asli", "Tidak ada tanda & di parameter, jadi ini pass by value"]}
  explanation="Karena tidak ada tanda & di parameter fungsi (int angka, bukan int& angka), ini adalah pass by value. Fungsi bekerja dengan salinan dari nilai, bukan variabel aslinya. Jadi 'nilai' di main() tetap 5 setelah pemanggilan fungsi."
  lang="id"
/>

<Exercise client:load
  id="u4l5-ex2"
  title="Fungsi Tukar dengan Reference"
  description="Lengkapi fungsi tukar agar benar-benar menukar isi dua variabel x dan y."
  type="code-output"
  expectedOutput="Sebelum tukar: x = 10, y = 20
Setelah tukar: x = 20, y = 10"
...
int main() {
  ...
  hints={["Fungsi tukar sudah menggunakan reference (int& a, int& b)", "Variabel temp digunakan untuk menyimpan nilai sementara", "Jalankan program — fungsi sudah benar!"]}
  explanation="Fungsi tukar menggunakan pass by reference (int& a, int& b), sehingga a dan b adalah alias dari x dan y di main(). Variabel temp menyimpan nilai a sementara, lalu a diisi b, dan b diisi temp. Hasilnya, x dan y benar-benar tertukar."
  lang="id"
/>
