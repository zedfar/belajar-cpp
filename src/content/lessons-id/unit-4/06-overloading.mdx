...
- `tampilkan(int angka)` — cetak angka
- `tampilkan(int angka, int kali)` — cetak angka sebanyak `kali` kali
- `tampilkan(const std::string& teks, char pembatas)` — cetak teks dengan pembatas di kiri dan kanan (misal `*Halo*`)

**Latihan 3:** Buat fungsi `konversi` yang di-overload:
- `konversi(double celsius)` — return Fahrenheit
- `konversi(double nilai, const std::string& dari)` — bisa konversi dari "km" ke meter, atau "kg" ke gram
