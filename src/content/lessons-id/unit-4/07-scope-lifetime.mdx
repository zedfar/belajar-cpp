...
<Exercise client:load
  ...
  description="Apa yang dicetak oleh program berikut? int x = 100; void fungsi() { int x = 50; std::cout &lt;&lt; x &lt;&lt; std::endl; } — saat fungsi() dipanggil dari main."
  type="multiple-choice"
  options={["100, karena variabel global diakses", "50, karena variabel lokal menutupi variabel global", "Error kompilasi karena nama variabel sama", "Hasilnya tidak terdefinisi"]}
  correctIndex={1}
  hints={["Name shadowing: variabel yang lebih dekat selalu menang", "Di dalam fungsi(), ada deklarasi int x = 50; yang bersifat lokal", "Variabel lokal menutupi (shadow) variabel global dengan nama sama"]}
  explanation="Ini adalah name shadowing. Di dalam fungsi(), deklarasi int x = 50 membuat variabel lokal x yang menutupi variabel global x = 100. Saat std::cout << x dieksekusi di dalam fungsi, variabel lokal yang digunakan, sehingga mencetak 50. Variabel global tetap 100 dan tidak terpengaruh."
  lang="id"
/>
...
<Exercise client:load
  ...
  description="Lengkapi program agar fungsi hitungPanggilan menggunakan static variable untuk menghitung berapa kali dipanggil."
  type="code-output"
  expectedOutput="Dipanggil: 1
Dipanggil: 2
Dipanggil: 3"
...
void hitungPanggilan() {
    ...
    counter++;
    std::cout << "Dipanggil: " << counter << std::endl;
}
...
int main() {
  ...
  hints={["static int counter = 0 hanya diinisialisasi sekali", "Setiap kali fungsi dipanggil, counter bertambah 1", "Nilai counter bertahan di antara pemanggilan karena keyword static"]}
  explanation="Variabel static local counter diinisialisasi hanya sekali (nilai 0) saat fungsi pertama kali dipanggil. Pada pemanggilan berikutnya, baris static int counter = 0 dilewati dan counter menggunakan nilai terakhirnya. Hasilnya counter bertambah terus: 1, 2, 3."
  lang="id"
/>
