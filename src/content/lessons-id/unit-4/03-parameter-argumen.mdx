...
<Exercise client:load
  ...
  description="Pada kode berikut: void sapa(std::string nama) { ... } — dan pemanggilan sapa('Andi'); — manakah yang disebut parameter dan mana yang disebut argumen?"
  type="multiple-choice"
  options={["nama adalah argumen, 'Andi' adalah parameter", "nama adalah parameter, 'Andi' adalah argumen", "Keduanya adalah parameter", "Keduanya adalah argumen"]}
  correctIndex={1}
  hints={["Parameter ada di definisi fungsi", "Argumen ada saat pemanggilan fungsi", "P = Parameter = Pembuatan fungsi. A = Argumen = Aksi pemanggilan"]}
  explanation="Parameter (nama) adalah variabel placeholder yang ditulis saat membuat fungsi di definisinya. Argumen ('Andi') adalah nilai aktual yang diberikan saat fungsi dipanggil. Cara mudah mengingatnya: Parameter ada di Pembuatan, Argumen ada di Aksi pemanggilan."
  lang="id"
/>
...
<Exercise client:load
  ...
  description="Lengkapi program agar menggunakan default parameter dan mencetak output yang benar."
  type="code-output"
  expectedOutput="Halo, Teman! Selamat datang!"
...
int main() {
    // Panggil sapa() tanpa argumen untuk menggunakan default
    sapa();
    return 0;
}`}
  hints={["Panggil sapa() tanpa memberikan argumen apapun", "Kalau tidak ada argumen, nilai default 'Teman' akan digunakan", "sapa(); — cukup tanda kurung kosong"]}
  explanation="Ketika fungsi sapa() dipanggil tanpa argumen, C++ menggunakan nilai default yang sudah ditentukan di definisi fungsi, yaitu 'Teman'. Hasilnya: 'Halo, Teman! Selamat datang!'"
  lang="id"
/>
